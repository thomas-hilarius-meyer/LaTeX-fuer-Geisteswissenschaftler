...
\minisec{Eingabe von Unicode-Zeichen in einem Emacs Buffer}

Zwei Methoden ermöglichen die Eingabe von Unicode-Zeichen in Emacs:

\minisubsec {Methode 1: Hexdezimalzahl eingeben}

Emacs-Dokumentation benutzt die Schreibweise: \texttt {C-x 8}
um zu beschreiben: man druckt die Tastenkombination
\KeyboardKey {Strg} und \KeyboardKey {8}.

In diesem Fall heisst das: %
\texttt {C-x 8} %
$\langle $\texttt {Return}$\rangle $ 
$\langle $\texttt {Unicode Codepoint}$\rangle $%
$\langle $\texttt {Return}$\rangle $ %
ermöglicht die direket Eingabe einer Unicode Zeichen.

Um einem ``Capitulum'' einzugeben: %
\texttt {C-x 8} %
$\langle $\texttt {Return}$\rangle $ 
\texttt {2E3F}%
$\langle $\texttt {Return}$\rangle $ %
...
\minisubsec {Methode 2: Emacs-Aliase benutzen}

Emacs hat eine lange Liste vorgefertigte Aliase um oft
benutzte Unicode Zeichen einzugeben. Zuerst, mit nur drei
Tastendrücke: um eine \texttt {no-break space} einzugeben: %
\texttt {C-x 8} %
$\langle $\texttt {Leertaste}$\rangle $ %

Einige Aliase sind fertig nach drei Tastendrücke. Andere
benötige vier Tastendrücke. Ich möchte das Wort ``Français''
eingeben. Mit %
\texttt {C-x 8 , c} %
kann ich einen ``c'' mit eine Cedille eingeben.

Ich möchte das Wort ``Mañana'' eingeben. Mit %
\texttt {C-x 8 ~ n} %
kann ich einen ``n'' mit einen Tilde eingeben.

Es gibt auch Aliase für drei verschiedene Bruchzahlen. Aber
\TeX{}-Benutzer haben nie Probleme damit gehabt, Bruchzahlen
zu setzen.

\minisec{Eingabe von Unicode-Zeichen in gedit}

gedit ermöglicht auch die direket Eingabe einer Unicode
Zeichen. Mit %
\texttt {S-C-u 2022} %
$\langle $\texttt {Return}$\rangle $ %
kann ich einen ``Bullet'' eingeben.

\minisec{Eingabe von Unicode-Zeichen in nano}

GNU nano ermöglicht auch die direket Eingabe einer Unicode
Zeichen.
...
nano erlaubt die Eingabe jede mögliche Zeichen (ausser
Meta-Sequenzen und Steuerung-Sequenzen) direkt in einem
Textdatei einzugeben.
...
$\langle $\texttt {Esc}$\rangle $%
-Taste drücken; %
$\langle $\texttt {dreistellige Dezimalzahl}$\rangle $

Methode 2:~\texttt {M-v} %
$\langle $\texttt {sechsstellige Hexadezimalzahl}$\rangle $%
