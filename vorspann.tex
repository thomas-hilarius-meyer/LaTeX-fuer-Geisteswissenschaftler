...
\lstdefinestyle{listinglfgw}{%
  ...
  morekeywords={%
  %a
  Afootnote,alteSeite,autopar,
  %b
  beginnumbering,Bfootnote,biblerefformat,biblerefmap,biblerefstyle,bonuspointpoints,
  %c
  Cfootnote,chapter,chbpword,chpword,chpgword,chqword,chsword,chtword,columnrulewidth,Columns,columnsposition,
  %e
  edtext,endnumbering,
  %f
  firstlinenum,firstlinenumR,firstpageheadrule,
  %h
  ...
  msdata,
  %p
  Pages,pend,pibibleverse,pointpoints,printbibliography,printindex,pstart,
  %q
  quell,question,
  %r
  Rcolwidth,
  %s
  selectlanguage,setlength,setmsdatalabel,solutiontitle,subsubsection,
  %t
  thealteSeite,
  %v
  vari,
...
\newtcblisting[use counter from=lfgwexample,number within=chapter]{lfgwcode}[1]{%
    ...
    listing only,
    title=Beispiel \thetcbcounter,
    listing options={style=listinglfgw,escapechar=§},
    #1,
}
...
%auskommentiert von Philipp, damit reledmac läuft; ist später ggf. nicht mehr nötig
%\usepackage{hyperxmp}
%\usepackage{hyperref}
...
\@ifpackagelater{reledmac}{2016/12/12}{%
    % Package is new enough
}{%
    \PackageError{reledmac}{Es wird reledmac >= 2.18.0b benötigt.}%
}
\makeatother
