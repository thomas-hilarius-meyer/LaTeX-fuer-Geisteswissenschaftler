...
\usepackage[german,polutonikogreek]{babel}
...
% grffile funktioniert nicht mit LuaLaTeX (Stand Oktober 2019)
% Workaround gemäß: 
% https://github.com/ho-tex/oberdiek/issues/73#issuecomment-543101052
%
\usepackage{grffile}
\usepackage{graphicx}
\graphicspath{{bilder/}}
...
\lstdefinestyle{listinglfgw}{%
  ...
  morekeywords={%
  %a
  addbibresource,addplot,Afootnote,alteSeite,autocite,autopar,answerline,alertblock,align,anchor,
  %b
  beginnumbering,Bfootnote,biblerefformat,biblerefmap,biblerefstyle,bonuspointpoints,bibleref,block,base,bottom,
  %c
  ...
  chronology,columns,coords,
  %d
  draw,dictum,deffootnote,deffootnote,description,dotted,dashed,
  %e
  edtext,endnumbering,enquote,enumi,example,exampleblock,edge,event,
  %f
  ...
  %h
  hpword,hqword,hsword,htword,hypertarget,hyperlink,hideallsubsection,height,
  %i
  includegraphics,includeonlyframes,ibibleref,invisible,
  %l
  Lcolwidth,ledsidenote,lemma,linenumincrement,linenumincrementR,label,
  %m
  msdata,mode,maketitle,multifootsep,midway,
  %n
  newfontface,newfontfamily,node,no,
  %o
  only,onslide,
  %p
  ...
  %r
  Rcolwidth,reledmac,red,
  %s
 selectlanguage,setlength,setmainfont,setmonofont,setmsdatalabel,setromanfont,setsansfont,smartcite,smartcites,solutiontitle,setbeamerfont,setbeamercovered,subsection,subsection*,subsubsection,subtitle,solution,solutionorgrid,style,
  %t
  ...
  textAlpha, textBeta, textGamma, textDelta, textEpsilon, textZeta, textEta, textTheta, textIota, textKappa, textLambda, textMu, textNu,
  textXi, textOmikron, textPi, textRho, textSigma, textTau, textUpsilon, textPhi, textChi,  textPsi, textOmega, twocolumn,tier,thick,
  %u
  usetheme,usecolortheme,usebackgroundtemplate,useforestlibrary,usepackage,usetikzlibrary,symbolic,
  %v
  vari,verse,very,
  %w
  word,width,
  %x
  Xafternumber,Xarrangement,Xlemmaseparator,Xinplaceoflemmaseparator,Xinplaceofnumber,Xnonbreakableafternumber,Xnotenumfont,Xnumberonlyfirstinline,Xnumberonlyfirstintwolines,Xsymlinenum,Xtwolines,Xtwolinesbutnotmore,Xtxtbeforenotes,xtick,
  %y
  ybar,ylabel,ytick
  },   
  literate=
...
\newtcblisting[use counter from=lfgwexample,number within=chapter]{lfgwcode}[1]{%
    ...
    listing only,
    title={Beispiel \thetcbcounter},
    listing options={style=listinglfgw,escapechar=§},%der escapechar soll bitte dableiben: #77 -- ph
    #1,
}
...
%	pdfdate={\the\year-\the\month-\the\day}, % Fehler
	pdflang={de},
...
%
% Float-Parameter einstellen
% Nur ändern, wenn es Probleme gibt
% (z.B. viele kleine Floats)
%
\renewcommand{\topfraction}{0.8} % 0.7
\renewcommand{\bottomfraction}{0.7} % 0.3
%\renewcommand{\textfraction}{0.1} % 0.2
%\renewcommand{\floatpagefraction}{0.7} 0.5
\setcounter{topnumber}{2} % 2
\setcounter{bottomnumber}{2} % 1
%\setcounter{totalnumber}{10} % 3

% Aus kritischeApparate
\newcounter{alteSeite}
