...
Während in den naturwissenschaftlichen Bereichen die Bibliographie eine weniger relevante Rolle spielt -- als bspw. in den Geisteswissenschaften -- hält sich dort noch die (veraltete) Technik Bibtex.
In den letzten Jahren hat sich jedoch allmählich und unumkehrbar die Möglichkeit Bibliographien und Zitate mittels Paket \paket{biblatex} durchgesetzt.\footcite{wassenhoven:dtk2008/2,wassenhoven:dtk2008/4}
Der Vorteil ist die leichte Einbindung und der Wechsel verschiedener Bibliographie- und Zitierstile, die als Option dem Paket übergeben werden. 

Das folgende Kapitel zeigt die Arbeit mit \paket{biblatex}: 
Zunächst wie man eine Bibliografie-Datenbank erstellt und verwaltet (\cref{sec:bibliografiedatenbank}),
wie man die notwendigen Felder bestimmter Eintragstypen (Buch, Zeitschrift, Lexikon etc.) belegt, wie man im eigenen Text die Verweise einabut (\cref{sec:zitate}) und
schließlich wie man eine oder mehrere (Teil-)Bibliografien erstellt.

Am Schluss werden verschiedene Bibliografie-Stile vorgestellt,
...
Die Zitation wird damit ›unsichtbar‹ zitiert:
